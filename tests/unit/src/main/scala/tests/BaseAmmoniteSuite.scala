...
abstract class BaseAmmoniteSuite(scalaVersion: String)
  ...
  test("hover") {
    for {
      ...
      _ = assert(
        newHoverRes == expectedNewHoverRes,
        s"Expected: '$expectedNewHoverRes', got: '$newHoverRes'"
      )
  ...
  test("simple errored script") {
    ...
    for {
      ...
      _ = expectDiagnostics("main.sc", expectedDiagnostics)
  ...
  private def expectDiagnostics(
    ...
    val diagnostics = server.client.pathDiagnostics(path)
    assert(
