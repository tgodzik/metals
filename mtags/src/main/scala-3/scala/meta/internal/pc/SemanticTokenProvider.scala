...
final class SemanticTokenProvider(
  ...
  def pickFromTraversed(
    ...
    def traillingCommaAdjust(pos: SourcePosition): Int =
      if pos.source.content.apply(pos.`end` - 1) == ',' then 1
      else 0
    def isTarget(node: NodeInfo): Boolean =
      node.pos.start == tk.pos.start &&
        (node.pos.end == tk.pos.end) ||
        (node.pos.end == tk.pos.end + traillingCommaAdjust(node.pos))
    ...
    candidates
      ...
      case manyNodes =>
        ...
        preFilter match
          case Nil => manyNodes.headOption.map(ni => (ni, candidates))
          case matchingNames
  ...
  override def identTypeAndMod(
    ...
    val ret =
      for (
        ...
        if sym.is(Flags.Abstract) || sym.isAbstractType then
