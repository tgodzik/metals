...
object SignatureHelpProvider:
  ...
  def contribute(driver: InteractiveDriver, params: OffsetParams) =
    ...
    val isUnapply = path.headOption.exists {
    ...
    new SignatureHelp(
      signatureInfos.map(signatureToSignatureInformation(_, isUnapply)).asJava,
      callableN,
      paramN
    )
  end contribute
  ...
  private def qual(tree: Tree): Tree =
    tree match
      case tpd.Apply(q, _) => qual(q)
      case tpd.TypeApply(q, _) => qual(q)
      case tpd.AppliedTypeTree(q, _) => qual(q)
      case tpd.Select(q, _) => q
      case _ => tree
  ...
  private def callInfo(path: List[Tree], span: Span)(using
    ...
    Signatures.callInfo(path, span)(using ctx) match
      case default @ (_, _, Nil) =>
        path match
          case (un @ (tpd.UnApply(fun, _, patterns))) :: parent :: _ =>
            ...
            val mtch = path.collectFirst { case t: tpd.Match =>
              t.selector
            }.get
            val (tpe, funSymbol) = seenFrom(mtch, fun.symbol)
            ...
            val cls = un.fun.denot.symbol.owner
            // val denot = symbol.asSeenFrom(tpe)
            // symbol.withUpdatedTpe(denot.info)
            val denot = signatureSymbol.asSeenFrom(signatureSymbol.owner.info)
  ...
  private def signatureToSignatureInformation(
    ...
    val label =
      if isUnapply then paramLists
      else s"${signature.name}$tparamsLabel$paramLists$returnTypeLabel"
