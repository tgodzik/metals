...
object MbtWorkspaceSymbolProvider {
  // Include CLASS symbols as BFS seeds only while the inheritance frontier is
  // small. Large frontiers are almost entirely leaf tests (`class FooTest
  // extends FunSuite`); searching the index for implementors of each of them
  // is the expensive case.
  private val MaxClassSeedFrontier = 256
...
class MbtWorkspaceSymbolProvider(
  ...
  def candidateMainClasses(
    ...
    for {
      ...
      if (fileCandidates.nonEmpty && filterPath(path)) {
   ...
   *
   * Inheritance BFS starts from `implementations` and, at later levels, prefers
   * traits/interfaces (custom test bases). Classes are included as seeds only
   * while the frontier is small, so JUnit 3 `TestCase` subclasses still chain.
   */
  private def transitiveReferenceFiles(
    ...
    while (frontier.nonEmpty) {
      ...
      val includeClasses =
        frontier.size <= MbtWorkspaceSymbolProvider.MaxClassSeedFrontier
      val nextBaseSymbols = frontier.flatMap { path =>
        bfsSeedSymbols(path, includeClasses)
      }.toSeq
  ...
  private def bfsSeedSymbols(
    ...
    documents.get(path).toSeq.flatMap { doc =>
      doc.symbols.collect {
        case sym
            if isBfsSeedKind(sym.getKind(), includeClasses) &&
  ...
  private def isBfsSeedKind(
    ...
    (includeClasses && kind == Semanticdb.SymbolInformation.Kind.CLASS)
  }
