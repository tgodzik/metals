...
final class ImplementationProvider(
  ...
  private def findImplementation(symbol: String): Set[ClassLocation] = {
    ...
    directImpl.toSet ++ directImpl
      .flatMap { loc =>
        findImplementation(loc.symbol).map(_.translate(loc))
      }
  }
    ...
    def calculateAsSeenFrom(t: TypeRef, classSig: ClassSignature) = {
      t.typeArguments.zipWithIndex.flatMap {
        case (arg: TypeRef, ind) =>
          // create mapping dependent on order - this way we don't need
  ...
  private case class ClassLocation(
    ...
    def translate(other: ClassLocation): ClassLocation = {
    ...
    // Translate postion based names to real names in the parent class
    def toRealNames(
        parentClass: SymbolInformation,
        translateKey: Boolean
    ): ClassLocation = {
      parentClass.signature match {
